(() => {
  ...
  const STORAGE_KEY = "ccx.state.v1";
  ...
  async function convert() {
    ...
    try {
      ...
      if (data?.date) {
        els.rateMeta.textContent = `ECB rate · ${data.date}`;
      } else {
        els.rateMeta.textContent = "ECB rate";
      }
      persist();
    } catch (err) {
      if (token !== convertToken) return;
  ...
  async function init() {
    try {
      ...
      if (typeof saved.amount === "string" && saved.amount !== "") {
        els.amount.value = saved.amount;
      } else {
      ...
      showError(
        "Couldn't load currencies. Please check your connection and retry.",
      );
    }
  }
